...
function jpegLSDecode(data: Uint8Array, isSigned: boolean) {
...
function initializeJPEGLS(): void {
...
function decodeJPEGLS(metaData: ImageData, pixelData: Uint8Array): ImageData {
  initializeJPEGLS();

  const image = jpegLSDecode(pixelData, metaData.pixelRepresentation === 1);
  ...
  metaData.rows = image.height;
  metaData.pixelData = image.pixelData;
